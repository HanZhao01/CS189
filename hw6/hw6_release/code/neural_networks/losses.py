...
class CrossEntropy(Loss):
    ...
    def forward(self, Y: np.ndarray, Y_hat: np.ndarray) -> float:
        ...
        # Clip `Y_hat` to max and min values (epsilon, 1-epsilon) to avoid log(0)
        Y_hat_clipped = np.clip(Y_hat, epsilon, 1 - epsilon)
        cross_entropy_loss = -np.sum(Y * np.log(Y_hat_clipped)) / Y.shape[0]
